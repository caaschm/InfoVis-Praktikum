...
export class TextViewerComponent implements OnInit, OnDestroy, AfterViewChecked, AfterViewInit {
  ...
  showChapterModal: boolean = false; // Show chapter creation modal
  newChapterType: string = 'chapter'; // Type of new chapter
  newChapterTitle: string = ''; // Custom title for new chapter
  newChapterEmoji: string = ''; // Emoji for new chapter
  ...
  /**
   * Open chapter creation modal
   */
  openChapterModal(): void {
    this.showChapterModal = true;
    this.newChapterType = 'chapter';
    this.newChapterTitle = '';
    this.newChapterEmoji = '';
  }

  /**
   * Close chapter creation modal
   */
  ...
  /**
   * Add a new section (shows form in sidebar)
   */
  addSection(): void {
  ...
  /**
   * Add a new chapter directly (creates a numbered chapter without modal)
   */
  addChapter(): void {
    if (!this.currentDocument) return;

    // Create a numbered chapter directly without opening the modal
    this.documentService.createChapter(
      this.currentDocument.id,
      undefined, // Title will be auto-generated by backend (e.g., "Chapter 1")
      'chapter',
      undefined // No emoji
    ).subscribe({
      next: (newChapter) => {
   ...
   */
  createChapterFromModal(): void {
    if (!this.currentDocument) return;
    
    // Determine title based on type
    let title = this.newChapterTitle;
    if (!title) {
      if (this.newChapterType === 'prologue') title = 'Prologue';
      else if (this.newChapterType === 'epilogue') title = 'Epilogue';
      else if (this.newChapterType === 'interlude') title = 'Interlude';
      else if (this.newChapterType === 'foreword') title = 'Foreword';
      else if (this.newChapterType === 'afterword') title = 'Afterword';
      else if (this.newChapterType === 'custom') title = 'Custom Section';
    ...
    this.documentService.createChapter(
      ...
      title || undefined,
      this.newChapterType,
      this.newChapterEmoji || undefined
    ).subscribe({
      next: (newChapter) => {
  ...
  /**
   * Common emojis for chapter selection
   */
  commonEmojis: string[] = [
    '📖', '📚', '📝', '✨', '🌟', '💫', '🔥', '💎',
    '⚔️', '🛡️', '👑', '🏰', '🌙', '☀️', '🌈', '🌊',
  ...
  startEditingChapter(chapter: Chapter): void {
    ...
    // For custom sections, use the title without numbers
  ...
  saveChapterTitle(chapter: Chapter): void {
    ...
    // Only chapters should be numbered - special sections (prologue, epilogue, etc.) should not
    if (chapter.type === 'chapter') {
      // For numbered chapters, preserve the chapter number format (01, 02, etc.)
      const chapterNum = (chapter.index + 1).toString().padStart(2, '0');
      newTitle = `${chapterNum} ${this.editingChapterTitle.trim()}`;
    } else {
      // For special sections (prologue, epilogue, afterword, etc.) and custom sections, use title as-is
  ...
  /**
   * Get formatted chapter title for display
   * Removes numbers from custom sections
   */
  ...
  getChapterTitleDisplay(chapter: Chapter): string {
    // Only chapters should have numbers - special sections should not
    if (chapter.type === 'chapter') {
      // For chapter title in text editor, show "01 Slay" format
      // Handle formats like "Chapter 1: Title" or "Chapter 1 Title"
   ...
   */
  onPlaceholderClick(chapterId: string | null, event?: Event): void {
    // Clear placeholder text immediately when clicked/focused
    const target = (event?.target || event?.currentTarget) as HTMLElement;
    if (target && (target.innerText === 'Start typing here...' || target.innerText.includes('Start typing here...'))) {
      target.innerText = '';
    ...
    // Set active chapter when user clicks placeholder
    if (chapterId) {
  ...
  onPlaceholderInput(event: Event, chapterId: string | null): void {
    ...
    // Remove placeholder text if it's still present (handle partial matches)
    const currentText = target.innerText;
    if (currentText.includes('Start typing here...')) {
      // Remove the placeholder text, keeping only user input
      target.innerText = currentText.replace(/Start typing here\.\.\./g, '').trim();
      // Move cursor to end
      const range = document.createRange();
      const selection = window.getSelection();
      range.selectNodeContents(target);
  ...
  onPlaceholderBlur(event: Event, chapterId: string | null): void {
    ...
    // CRITICAL: Remove any placeholder text that might still be present
    text = text.replace(/Start typing here\.\.\./g, '').trim();
    
    if (text && this.currentDocument && chapterId) {
