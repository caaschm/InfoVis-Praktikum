...
from app.services.ai_client import generate_chapter_title, generate_chapter_emoji
...
@router.post("/{chapter_id}/suggest-emoji", response_model=schemas.ChapterEmojiSuggestion)
async def suggest_chapter_emoji(
    document_id: str,
    chapter_id: str,
    db: Session = Depends(get_db)
):
    """
    Generate an AI-suggested emoji for a chapter based on its content.
    """
    # Verify document and chapter exist
    document = db.query(models.Document).filter(models.Document.id == document_id).first()
    if not document:
    ...
    suggested_emoji = await generate_chapter_emoji(chapter_content)
    
    return {"suggested_emoji": suggested_emoji}
